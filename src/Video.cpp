...
#ifdef m5stack
#define PIXELMATRIX_W (280)
#define PIXELMATRIX_H (192 + 2 * HGR_MATRIX_YOFFSET)
static BYTE          *hgrpixelmatrix;
#define GetPixelMatrix(x,y) (hgrpixelmatrix[(x)*PIXELMATRIX_W+(y)])
#define SetPixelMatrix(x,y,v) (hgrpixelmatrix[(x)*PIXELMATRIX_W+(y)] = v)
#else /* m5stack */
...
//  g_hDeviceDC = CreateCompatibleDC(dc);
  ...
  if (g_hDeviceBitmap)
    ...
    //g_hDeviceBitmap = SDL_CreateRGBSurface(SDL_SWSURFACE, 280, 192, 8, 0, 0, 0, 0);
...
void /*__stdcall */ MixColorsVertical(int matx, int maty) {	// For tv emulation g_nAppMode
  ...
  if (SW_MIXED && maty > 159) {
	  if (maty < 161) {
        bot1idx = GetPixelMatrix(matx,maty+1) & 0x0F;
        bot2idx = 0;
	  } else {
        bot1idx = bot2idx = 0;
	  }
    ...
    bot1idx = GetPixelMatrix(matx,maty+1) & 0x0F;
    bot2idx = GetPixelMatrix(matx,maty+2) & 0x0F;
  }

  twoHalfPixel = colormixmap[GetPixelMatrix(matx,maty-2) & 0x0F]
	                        [GetPixelMatrix(matx,maty-1) & 0x0F]
                            [GetPixelMatrix(matx,maty) & 0x0F];
  colormixbuffer[0] = (twoHalfPixel & 0xFF00) >> 8;
  colormixbuffer[1] = twoHalfPixel & 0x00FF;

  twoHalfPixel = colormixmap[GetPixelMatrix(matx,maty-1) & 0x0F]
	                        [GetPixelMatrix(matx,maty) & 0x0F]
                            [bot1idx];
  colormixbuffer[2] = (twoHalfPixel & 0xFF00) >> 8;
  colormixbuffer[3] = twoHalfPixel & 0x00FF;

  twoHalfPixel = colormixmap[GetPixelMatrix(matx,maty) & 0x0F]
	                        [bot1idx]
                            [bot2idx];
...
void /*__stdcall */ CopyMixedSource (int x, int y, int sourcex, int sourcey) {	// For tv emulation g_nAppMode
  ...
  for (count = 0, bufxoffset = 0;
       count < 7;
	   count++, bufxoffset += 2) {
    SetPixelMatrix(matx+count,maty, *(currsourceptr+bufxoffset));
...
#ifdef m5stack
  info("VideoInitialize");
  hgrpixelmatrix = (BYTE*) VirtualAlloc(NULL, PIXELMATRIX_W * PIXELMATRIX_H * sizeof (BYTE), 0, 0);
  if (!hgrpixelmatrix) {
    error("can't malloc hgrpixelmatrix");
  }
#endif /* m5stack */
...
void VideoRefreshScreen () {
  ...
  //  LONG/*int*/   pitch   = 280; // pitch stands for pixels in a row, if one pixel stands for one byte (560 in our case)
