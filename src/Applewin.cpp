...
#ifdef m5stack
	g_bFullSpeed = ( (g_dwSpeed == SPEED_MAX) || bScrollLock_FullSpeed || (DiskIsSpinning() && enhancedisk && !Spkr_IsActive()) );
#else /* m5stack */
...
void LoadConfiguration ()
{
  ...
  g_dwSpeed = 40; // fastest

  enhancedisk = 1; // enhanced disk speed
  videotype = 1; // Color Standard
  ...
  g_uMouseInSlot4 = 0; // off

  //g_bSaveStateOnExit = false;
  ...
  //DoDiskInsert(0, "Master.dsk");
  if (0) {
    strncpy(g_sParallelPrinterFile, "", MAX_PATH);
  }
...
int _main(int argc, char * lpCmdLine[])
{
