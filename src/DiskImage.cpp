               ...
               BOOL     bCreateIfNecessary)
{
  ...
  printf("format=%d\n", format);
  ImageInfoRec *imageinfo = new ImageInfoRec;
  ZeroMemory(imageinfo, sizeof(ImageInfoRec));
  strncpy(imageinfo->filename, imagefilename, MAX_PATH);
  imageinfo->format = format;
  imageinfo->file = pImage;
  imageinfo->offset = 0;
  imageinfo->writeprotected = *pWriteProtected_;
  for (int track = 0; track < TRACKS; track++) {
    imageinfo->validtrack[track] = (size > 0);
  }
  *hDiskImage_ = (HIMAGE)imageinfo;
